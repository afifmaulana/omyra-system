<div class="bottom-nav">
    <div class="d-flex justify-content-around">
        ...
        <a class="text-center py-2 m-0 {{ request()->routeIs(['frontend.stock.index']) ? 'active' : '' }}" href="{{ route('frontend.stock.index') }}">
            {{-- <img src="{{ asset('images/icon/forme.png') }}" alt=""> --}}
            <i class="fa fa-archive h-40 p-0 m-0"></i>
            <p class="font-xs text-light-pink p-0 m-0 text-center {{ request()->routeIs(['frontend.stock.index']) ? 'text-active-pink' : '' }}">Stok <br>Box</p>
        </a>
        @endif
        @if (Auth::user()->role == '1' || Auth::user()->role == '0')
        <a class="text-center py-2 m-0 {{ request()->routeIs(['frontend.semi-finished.index']) ? 'active' : '' }}" href="{{ route('frontend.semi-finished.index') }}">
            {{-- <img src="{{ asset('images/icon/forme.png') }}" alt=""> --}}
            <i class="fa fa-cube h-40 p-0 m-0"></i>
            <p class="font-xs text-light-pink p-0 m-0 text-center {{ request()->routeIs(['frontend.semi-finished.index']) ? 'text-active-pink' : '' }}">Barang <br>1/2 Jadi</p>
        </a>
        <a class="text-center py-2 m-0 {{ request()->routeIs(['frontend.finished.index']) ? 'active' : '' }}" href="{{ route('frontend.finished.index') }}">
            {{-- <img src="{{ asset('images/icon/forme.png') }}" alt=""> --}}
            <i class="fa fa-cubes h-40 p-0 m-0"></i>
            <p class="font-xs text-light-pink p-0 m-0 {{ request()->routeIs(['frontend.finished.index']) ? 'text-active-pink' : '' }}">Barang<br> Jadi</p>
        </a>
        @endif
